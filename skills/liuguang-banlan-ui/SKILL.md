...
description: Build, redesign, or review parameterized front-end interfaces in two fixed visual modes—流光溢彩白 and 五彩斑斓黑—with OKLCH color contracts, a continuous WebGL/CSS spectral field, responsive workbench structure, visual-capability gating, screenshot QA, and deterministic pixel reports. Use when a user asks for either named style, a unified skill covering both styles, a reusable starter, color-intensity parameters, screenshot measurement, or a final report listing total and per-color parameters.
---
